...
export default function IpAnalysis() {
  ...
  const handleScan = async (e) => {
    ...
    try {
      const response = await api.post('/analyze/ip', { ip: ipInput.trim() });
      ...
      setError(err.response?.data?.error?.message || err.message || 'An error occurred during IP reputation lookup');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <h1 className="text-xl font-bold text-text-primary">IP reputation Lookup</h1>
        <p className="text-text-muted text-xs">Query IP addresses against AbuseIPDB confidence indices, VirusTotal engines, and ThreatMiner passive DNS records</p>
      </div>
